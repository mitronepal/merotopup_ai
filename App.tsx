...
const App: React.FC = () => {
    ...
    const init = async () => {
      ...
      if (savedUid) {
        ...
        if (user) {
          ...
          addMessage('assistant', `स्वागत छ ${user.username}! 🙏 विशाल घिमिरेको डिजिटल सेल्सम्यान यहाँ छ। आज कुन टप-अप गरौँ? 🎮`);
          return;
        }
      }
      addMessage('assistant', `नमस्ते! 🙏 म विशाल घिमिरे (Bishal Ghimire) द्वारा निर्मित "MeroTopup AI" हुँ।\n\nकुनै समस्या परेमा उहाँलाई सिधै **9764630634** मा सम्पर्क गर्नुहोला।\n\nसुरु गर्न कृपया आफ्नो **Email Address** राख्नुहोस्:`, undefined, 'AUTH_EMAIL');
    };
    init();
  }, []);

  const addMessage = (role: 'user' | 'assistant', content: string, image?: string, interactiveType?: Message['interactiveType'], data?: any) => {
    setMessages(prev => [...prev, { id: Date.now().toString() + Math.random(), role, content, timestamp: new Date(), image, interactiveType, data }]);
  };
  ...
  const handleAuth = async () => {
    ...
    if (lastMsgType === 'AUTH_EMAIL') {
      ...
      if (user) {
        addMessage('assistant', `पासवर्ड राख्नुहोस्:`, undefined, 'AUTH_PASSWORD');
      } else {
        addMessage('assistant', `तपाईंको पूरा नाम के हो?`, undefined, 'REGISTER_NAME');
      }
    } else if (lastMsgType === 'REGISTER_NAME') {
      if (!auth.name.trim()) return;
      addMessage('user', auth.name);
      addMessage('assistant', `नयाँ पासवर्ड बनाउनुहोस्:`, undefined, 'AUTH_PASSWORD');
    } else {
      ...
      try {
        ...
        if (user && (auth.name || (user as any).password === auth.password)) {
          ...
          const orders = await firebaseService.getUserOrders(user.userId);
          setOrderHistory(orders);
          addMessage('assistant', `लग-इन सफल! कुन गेम टप-अप गर्ने?`);
        } else throw new Error("AUTH_FAILED");
      } catch(e) { 
        addMessage('assistant', "पासवर्ड मिलेन। समस्या परेमा विशाललाई ९७६४६३०६३४ मा भन्नुहोला।", undefined, 'AUTH_PASSWORD'); 
      }
      setIsLoading(false);
    }
  };
  ...
  const handleSend = async (customMsg?: string) => {
    ...
    try {
      ...
      let errorMsg = "सर्भरमा समस्या आयो। कृपया विशाल घिमिरेलाई ९७६४६३०६३४ मा सिधै सम्पर्क गर्नुहोला।";
      if (e.message.includes("MISSING_API_KEY")) {
        errorMsg = "API_KEY सेट गरिएको छैन। कृपया सर्भरको Environment Variables चेक गर्नुहोस्।";
      } else if (e.message.includes("RATE_LIMIT")) {
        errorMsg = "धेरै ट्राफिक भयो। कृपया केही मिनेटपछि प्रयास गर्नुहोस् वा विशाललाई ९७६४६३०६३४ मा भन्नुहोस्।";
      }
  ...
  const handleFileUpload = async (e: any) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        if (result.valid) {
          addMessage('assistant', `पेमेन्ट सफल! ✅ अर्डर पेन्डिङमा छ, विशालले चाँडै टप-अप गरिदिनुहुनेछ।\nID: ORD${Date.now().toString().slice(-6)}`);
        ...
        addMessage('assistant', "रसिद विश्लेषणमा समस्या आयो। विशाललाई ९७६४६३०६३४ मा सम्पर्क गर्नुहोला। ❌"); 
      }
      setIsLoading(false);
    };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-1.5">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Bishal's Official AI</span>
            </div>
          </div>
        </div>
              ...
              {orderHistory.length === 0 ? (
                ...
                >
                  <div className="space-y-1">
                    <p className="font-black text-indigo-600 text-[9px] uppercase tracking-widest">#{o.orderId.split('-').pop()}</p>
